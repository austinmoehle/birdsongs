...
def process_raw_master_set(directories, target, match='Song.mp3', dummy=True,
                           bird_list_path='bird_list.txt',
                           counts_path='counts.txt'):
    """Processes Master Set recordings in 'directories'.
    ...
    bird_list = sorted(bird_counts.keys(), key=lambda x: bird_counts[x], reverse=True)
    if dummy:
...
def trim_audio(data, rate=44100, start_trim=0, end_trim=0, log=False):
    ...
    if log:
        ...
        h, m = divmod(m, 60)
        print("Original recording length: %d h %d m %d s" % (h, m, s))
        print("Removed [%d s, %d s] from [start, end] of recording." %
              (start_trim, end_trim))
    return chop
...
def find_loudest_subset(data, subset_length, step=0.1, rate=44100):
    ...
    weights = abs_weights(subset_samples)
    for i in xrange(int((len(data) - subset_samples + 1.0)/step_samples)):
        idx = i * step_samples
...
def snip_audio(data, snip_length=4, cutoff=0.25, min_snips=None, max_snips=None,
    ...
    if log is True:
        print("Data padded with %.1f s of zeros (half a snippet length) on both ends." %
              (float(snip_length)/2))
    snips = []
    logs = []
    max_sum = 0
...
def plot_spectrograms(data, rate=44100, num_figures=1, fig_size=(16,9),
    ...
    for i in xrange(num_figures):
        ...
        plt.tight_layout()
    plt.show()
    print '[Time subdivisions, frequency subdivisions]:'
    print shapes
...
def compute_spectrograms(data, rate=44100, fig_size=(16,9), sample_index=None,
    ...
    try:
        ...
        print 'Spectrogram index error. Skipping this spectrogram.'
        continue
    log_S = librosa.logamplitude(S, ref_power=np.max)
    ...
    for i, collection in enumerate(snippets):
        for j, snip in enumerate(collection):
            if first:
                ...
                first = False
            spectrograms = compute_spectrograms(snip, num_figures=num_figures)
            for k in xrange(len(spectrograms[0])):
                label = str(bird_index) + '_' + str(rec_index)
                label += '_i%dj%dk%dc%d' % (i, j, spectrograms[1][k],
                                            int(100.*logs[i][0]))
...
def add_snippets_from_file(path, target, sample_dir, bird_index, rec_index,
                           ...
                           num_figures=None, num_jitters=None, jitter=0.25,
                           train=True):
    """Adds snippets from an audio file."""
    rate, data = wav_to_raw(path)
    if rate != 44100:
        print('Rate is not 44100 Hz (%s Hz)' % str(rate))
...
def make_snippets(snip_dir, snip_length=4, num_jitters=10, jitter=0.5):
    ...
    for subd in os.listdir(snip_dir):
        ...
        for rec in os.listdir(bird_path):
            ...
            add_snippets_from_file(rec_file, rec_dir, sample_dir, index, curr,
                                   ...
                                   jitter=jitter)
            curr += 1
        print('Finished with %d recordings for bird %d' % (curr, index))
...
def snips_to_split(snip_dir, data_dir, test_recs=2, min_recs=6,
    ...
    num = sum([len([f for f in os.listdir(os.path.join(train_dir, sd))])
              for sd in subs])
    print("Number of images in train split: %d" % num)
    ...
    num = sum([len([f for f in os.listdir(os.path.join(test_dir, sd))])
              for sd in subs])
    print("Number of images in test split: %d" % num)

    # Get 'bird_list' and use it to write the labels file for Inception-v3 training.
    with open(bird_list_path) as f:
        lines = f.readlines()
    bird_list = [line.rstrip('\n') for line in lines]
    labels_to_class_names = {label: bird for label, bird in
                             enumerate(bird_list) if label <= cutoff}
    write_labels_file(labels_to_class_names, data_dir, filename=labels_file_path)
